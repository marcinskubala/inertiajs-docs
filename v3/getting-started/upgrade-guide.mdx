...
<Columns cols={2}>
  ...
  <Card
    ...
    href="/v3/data-props/merging-props"
    icon="refresh"
  >
    Seamlessly load more data as the user scrolls down the page.
  </Card>
  <Card
    title="Lazy loading data on scroll"
    href="/v3/data-props/load-when-visible"
    icon="mouse"
  >
  ...
  <Card
    ...
    href="/v3/security/history-encryption"
    icon="lock"
  >
    Clear page data from browser history state when logging out of an
    application.
  </Card>
</Columns>
...
The previously deprecated `router.replace` method has been re-instated, but its functionality has changed. It is now used to make [Client Side](/v3/the-basics/manual-visits#client-side-visits) page visits. To make server-side visits that replace the current history entry in the browser, use the `replace` option:
...
- Updated `setup` callback in `app.js`. You need to pass `props` when initializing the `App` component. [See setup in app.js](/v3/installation/client-side-setup#initialize-the-inertia-app)
- `setup` callback is now required in `ssr.js`. [See setup in ssr.js](/v3/advanced/server-side-rendering#add-server-entry-point)
